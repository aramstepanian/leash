...
enum LeashMenuMode: Equatable {
    ...
    var accessibility: String {
        ...
        case .working: return LeashCopy.runningA11y
...
struct LeashMenuBarLabel: View {
    ...
    var body: some View {
        Image(filled ? "LeashMenuFilled" : "LeashMenu")
            .renderingMode(.template)
            .interpolation(.high)
            .opacity(opacity)
            .onAppear { pulse = animating }
            .onChange(of: animating) { _, on in pulse = on }
            .animation(animating ? LeashMotion.pulse : LeashMotion.settle, value: pulse)
            .accessibilityLabel(mode.accessibility)
    }

    private var filled: Bool {
        switch mode {
        case .watching, .working, .waiting, .connecting: return true
    ...
    private var animating: Bool {
        switch mode {
        case .working, .waiting, .connecting: return true
        case .idle, .watching, .offline: return false
        }
    }

    private var opacity: Double {
        if mode.dimmed { return LeashPaint.Opacity.disabled }
        if animating && pulse { return LeashPaint.Opacity.pulse }
        return 1
    }
}
        ...
        Canvas { context, size in
            ...
            case .fasten:
                ...
                drawArc(
                    ...
                    tint: tint,
                    start: .degrees(-90),
                    sweep: .degrees(220 + 140 * Double(seated))
                )
